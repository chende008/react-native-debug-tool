...
export default class DebugManager {
    ...
    static appendLogs(text) {
        if (debugLogOff) return;
        costTimeWork(() => {
            normalLogs.unshift({log: text, timeStr: dateFormat(new Date(), 'MM月dd日 hh时mm分ss秒')});
            if (normalLogs.length > 30) normalLogs.splice(20)//若url个数大于30，则删除前20个
        })
    }
    ...
    static showFloat(sibling) { // sibling => RootSiblings instance
        debugLogOff = false;
        if (sibling) {
            sibling.update(<FloatPanelController close={() => sibling.destroy()}/>)
        } else {
            const RootSibling = require('react-native-root-siblings');
            this.sibling = new RootSibling(<FloatPanelController close={() => this.sibling.destroy()}/>)
        }
    }
}
