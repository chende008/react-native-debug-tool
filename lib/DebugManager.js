...
let httpRequestLogs = [], webViewLoadLogs = [], logTextArr = [];
...
export default class DebugManager {
    ...
    static logTextArr(text) {
        if (!this.logOn) return;
        costTimeWork(() => {
            logTextArr.unshift({log: text, timeStr: dateFormat(new Date(), 'MM月dd日 hh时mm分ss秒')});
            if (logTextArr.length > 30) logTextArr.splice(20)//若url个数大于30，则删除前20个
        })
    }
