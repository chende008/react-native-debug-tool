...
export default class FloatPanelController extends PureComponent {
    ...
    renderPageFirst = (currentUrl) => {
        ...
        return <View style={{flex: 1, width: screenWidth, backgroundColor: Colors.white}}>
            ...
            {!isEmpty(currentUrl) && <DebugItem title='环境切换' text={currentUrl} style={{color: Colors.red, fontSize: 13}} onPress={() => this.changeToDetail(1, 'EnvironmentChange', '服务器环境切换')} showLine/>}            {!isEmpty(DebugManager.getLogText()) && <ScrollView style={{marginVertical: 15}}>{
            DebugManager.getLogText().map((item, index) => {
                return <View key={index} style={styles.borderStyle}>
                    <Text style={styles.title} onPress={() => showMsg(item.log)}>
                        <Text style={{fontWeight: 'bold'}}>日志：</Text>{item.log}
                    </Text>
                    <Text style={styles.title}>
                        <Text style={{fontWeight: 'bold'}}>时间：</Text>{item.timeStr}
                    </Text>
                </View>
            })}
        </ScrollView>}
